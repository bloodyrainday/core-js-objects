...
function compareObjects(obj1, obj2) {
  ...
  return obj1Keys.length === res.length;
}
...
function makeWord(lettersObject) {
  ...
  Object.keys(lettersObject).forEach((key) => {
    sumOfValues += lettersObject[key].length;
  });
  const res = Array(sumOfValues);
  Object.entries(lettersObject).forEach((obj) => {
    const [key, arrOfValues] = obj;
    arrOfValues.forEach((val) => {
      res.splice(val, 1, key);
    });
  });
  return res.join('');
}
...
function Rectangle(width, height) {
  this.width = width;
  this.height = height;
  this.getArea = function culcArea() {
...
function fromJSON(proto, json) {
  ...
  Object.entries(parse).forEach((obj) => {
    const [key, value] = obj;
    res[key] = value;
  });
  return res;
